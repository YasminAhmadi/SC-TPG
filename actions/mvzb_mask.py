...
class ActionExecutor:
    """
      - action side reads the SAME observed enemies produced by feature side:
            bot._mvzb_obs_enemies_by_tag[tag]
    ...
    def _should_activate_combat_mode(self, bot, marines, enemies) -> bool:
        ...
        activate_radius = float(self._cfg_val(bot, "mvzb_combat_activate_radius", 12.0))
    ...
    def _fallback_local_observed_enemies(self, bot, marine) -> List[object]:
        ...
        """
        combat_mode = bool(getattr(bot, "_mvzb_combat_mode", False))

        # if cache absent and combat mode still False, try to infer once
        if not combat_mode:
            marines = list(marine_units(bot))
            enemies_all = list(getattr(bot, "enemy_units", []))
            if self._should_activate_combat_mode(bot, marines, enemies_all):
                combat_mode = True
                bot._mvzb_combat_mode = True

        search_obs_radius = float(self._cfg_val(bot, "mvzb_search_obs_radius", 999.0))
        combat_obs_radius = float(self._cfg_val(bot, "mvzb_combat_obs_radius", 10.0))
    ...
    def _get_observed_enemies_for_unit(self, bot, marine) -> List[object]:
        ...
        cached_loop = int(getattr(bot, "_mvzb_obs_game_loop", -1))
        obs_by_tag = getattr(bot, "_mvzb_obs_enemies_by_tag", None)
    ...
    async def apply_actions(self, bot, marine_actions: Dict[int, int]) -> float:
        ...
        for idx, m in enumerate(ms):
            ...
            # critical: use observed enemies, not full real enemies
            ...
            # combat on observed enemies only
    ...
    def _select_target(self, enemies, px: float, py: float, tgt_idx: int) -> Tuple[Optional[object], float]:
        ...
        if tgt_idx == 3: # highest threat among observed enemies only
            def threat(u) -> int:
                try:
