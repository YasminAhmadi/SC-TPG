...
# from tpg.exp3_router import EXP3_ROUTER
...
class Team:
    ...
    def act(self, state, visited, actVars=None, path_trace=None):
        ...
        if not valid_learners:
            atomics = [lr for lr in self.learners if lr.isActionAtomic()]
            if atomics:
                top_learner = max(atomics, key=lambda lr: lr.bid(state, actVars=actVars))
                return top_learner.getAction(state, visited=visited, actVars=actVars, path_trace=path_trace)
            # last resort random primitive action
            nA = (actVars or {}).get("n_actions", 1)
            return random.randrange(nA)

        top_learner = max(valid_learners,
                          key=lambda lrnr: lrnr.bid(state, actVars=actVars))
        # print("top_learner.bid(state, actVars=actVars):", top_learner.bid(state, actVars=actVars))
        # # --- NEW: compute bids once, then EXP3 sample among valid learners
        # pairs = []
        # bids_cache = {}
        # for lr in valid_learners:
        #     b = lr.bid(state, actVars=actVars)
        #     bids_cache[lr.id] = b
        #     pairs.append((lr.id, b))
        #
        # chosen_id, prob = EXP3_ROUTER.sample(self.id, pairs)
        # top_learner = next(lr for lr in valid_learners if lr.id == chosen_id)
        #
        # # path trace (unchanged except we reuse cached bids to avoid recomputing)
        # if path_trace is not None:
        #     last_segment = path_trace[-1] if len(path_trace) != 0 else None
        #     path_segment = {
        #         'team_id': str(self.id),
        #         'top_learner': str(top_learner.id),
        #         'top_bid': bids_cache[top_learner.id],
        #         'top_action': top_learner.actionObj.actionCode if top_learner.isActionAtomic()
        #         else str(top_learner.actionObj.teamAction.id),
        #         'depth': last_segment['depth'] + 1 if last_segment is not None else 0,
        #         'bids': []
        #     }
        #     for lr in valid_learners:
        #         path_segment['bids'].append({
        #             'learner_id': str(lr.id),
        #             'bid': bids_cache[lr.id],
        #             'action': lr.actionObj.actionCode if lr.isActionAtomic()
        #             else str(lr.actionObj.teamAction.id)
        #         })
        #     path_trace.append(path_segment)
        #
        # return top_learner.getAction(state, visited=visited, actVars=actVars, path_trace=path_trace)

        # Old way of tracing path
    ...
    def mutate(self, mutateParams, allLearners, teams):
        ...
        for i in range(rampantReps):
            ...
            # with maxTeamSize：
            added_learners = self.mutation_add(
