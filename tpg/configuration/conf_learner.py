# tpg\configuration\conf_learner.py
import uuid
...
class ConfLearner:
    ...
    def init_def(self, initParams, program, actionObj, numRegisters, learner_id=None):
        ...
        self.actionObj = ActionObject(action=actionObj,
                                      initParams=initParams)  # Each learner should have their own copy of the action object
    ...
    def bid_def(self, state, actVars=None):
        ...
        if not np.isfinite(bid):
            ...
            # clip
            bid = max(min(bid, 1e3), -1e3)

        self.registers[0] = bid
        return bid
