# tpg\configuration\conf_team.py
import random
import uuid
import math
"""
...
def _safe_bid(lrnr, state, actVars=None, default=-1e18):
    try:
        b = lrnr.bid(state, actVars=actVars)
        b = float(b)
        return b if math.isfinite(b) else default
    except Exception:
        return default
...
class ConfTeam:
    ...
    def act_def(self, state, visited, actVars=None, path_trace=None):
        ...
        # 1) If revisited: Do not raise, let training continue (otherwise SC2 will crash).
        if str(self.id) in visited:
            # Infinite loop protection: Returns a safe atomic action (which will later be clipped by env_action_space.clip).
            if path_trace is not None:
                path_trace.append({
                    ...
                    'top_bid': None,
                    'top_action': 0,
                    'depth': (path_trace[-1]['depth'] + 1) if len(path_trace) else 0,
                    'bids': [],
                    'note': 'revisited_team_fallback'
                })
            return 0

        visited.append(str(self.id))

        # 2) filtering: atomic or teamAction not accessed.
        valid_learners = [
            lrnr for lrnr in self.learners
            if lrnr.isActionAtomic() or str(lrnr.getActionTeam().id) not in visited
        ]

        # 3) when valid_learners empty，do not max([])
        if len(valid_learners) == 0:
            # self team's atomic learners
            atomic_learners = [lrnr for lrnr in self.learners if lrnr.isActionAtomic()]
            if len(atomic_learners) > 0:
                top_learner = max(atomic_learners, key=lambda lrnr: _safe_bid(lrnr, state, actVars))
                if path_trace is not None:
                    path_trace.append({
                        'team_id': str(self.id),
                        'top_learner': str(top_learner.id),
                        'top_bid': top_learner.bid(state, actVars=actVars),
                        'top_action': top_learner.actionObj.actionCode,
                        'depth': (path_trace[-1]['depth'] + 1) if len(path_trace) else 0,
                        'bids': [{
                            'learner_id': str(l.id),
                            'bid': l.bid(state, actVars=actVars),
                            'action': l.actionObj.actionCode
                        } for l in atomic_learners],
                        'note': 'empty_valid_learners_use_atomic'
                    })
            ...
            if path_trace is not None:
                path_trace.append({
                    ...
                    'bids': [{
                        'learner_id': str(l.id),
                        'bid': l.bid(state, actVars=actVars),
                        'action': (l.actionObj.actionCode if l.isActionAtomic() else str(l.actionObj.teamAction.id))
                    } for l in self.learners],
                    'note': 'empty_valid_learners_return_default_action'
                })
            return 0

        # 4)  top learner
        top_learner = max(valid_learners, key=lambda lrnr: _safe_bid(lrnr, state, actVars))
        ...
        if path_trace is not None:
            ...
            path_segment = {
                ...
                'depth': last_segment['depth'] + 1 if last_segment is not None else 0,
                'bids': []
            }
    ...
    def act_learnerTrav(self, state, visited, actVars=None, path_trace=None):
        ...
        if path_trace != None:
            ...
            path_segment = {
                ...
                'top_learner': str(top_learner.id),
                'top_bid': top_learner.bid(state, actVars=actVars),
                'top_action': top_learner.actionObj.actionCode if top_learner.isActionAtomic() else str(
                    top_learner.actionObj.teamAction.id),
            ...
            for cursor in valid_learners:
                path_segment['bids'].append({
                    'learner_id': str(cursor.id),
                    'bid': cursor.bid(state, actVars=actVars),
                    'action': cursor.actionObj.actionCode if cursor.isActionAtomic() else str(
                        cursor.actionObj.teamAction.id)
                })
    ...
    def removeLearner_def(self, learner):
        ...
        if len(to_remove) != 1:
            raise Exception(
                "Duplicate learner detected during team.removeLearner. {} duplicates".format(len(to_remove)))
    ...
    def mutate_def(self, mutateParams, allLearners, teams):
        ...
        if (mutateParams["rampantGen"] > 0 and  # The rapantGen cannot be 0, as x mod 0 is undefined
                mutateParams["generation"] % mutateParams[
                    "rampantGen"] == 0 and  # Determine if this is a rampant generation
                mutateParams["generation"] > mutateParams["rampantGen"]
                # Rampant generations cannot occur before generation passes rampantGen
        ):
            rampantReps = random.randrange(mutateParams["rampantMin"], mutateParams["rampantMax"]) if mutateParams[
                                                                                                          'rampantMin'] < \
                                                                                                      mutateParams[
                                                                                                          'rampantMax'] else \
        ...
        for i in range(rampantReps):
            ...
            '''
            # print("conf_team mutation_delete")
            ...
            # print("mutateParams[pLrnMut]:", mutateParams["pLrnMut"])#
            # print("mutateParams[pActMut]:", mutateParams["pActMut"])
            mutated_learners, mutation_added_learners = self.mutation_mutate(mutateParams["pLrnMut"], mutateParams,
                                                                             teams)
