...
class SwarmBot(BotAI):
    ...
    async def on_start(self):
        ...
        # else:
        #     # first time start, or no valid resume state found
        #     await self._start_new_generation(first=True)
    ...
    async def _start_new_generation(self, first: bool = False):
        ...
        if first:
            ...
            # fresh start of a new generation
            self.generation = gen_on_trainer
    ...
    def _start_new_episode_counters(self):
        ...
        # for new episode, we need to clean up stray marines once, to avoid them interfering with termination conditions
    ...
    async def _finish_episode_for_current_agent(self, reason: str, already_reset: bool = False, final_snapshot: Optional[dict] = None):
        ...
        # not finished all episodes for this agent yet
        ...
        # all episodes for this agent finished
        self.cur_idx += 1

        # eval all agents finished: evolve and start new gen
    ...
    async def on_step(self, iteration: int):
        ...
        # First frame: Clearing away the remaining wild units from the previous round.
