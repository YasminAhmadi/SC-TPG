...
class SwarmBot(BotAI):
    def __init__(self, trainer: Trainer, task, policy, cfg: TrainConfig):
        ...
        self._rew_ov_final = None # auto_reset_jump 

        # find zergling
        self._enemy_tag2type = {}# tag -> UnitTypeId
        self._kills_this_ep = 0
        self._marine_deaths_this_ep = 0
        self._prev_kills_this_ep = 0
        self._prev_marine_deaths_this_ep = 0
        ###
    ...
    def _dead_unit_tags(self):

        du = getattr(getattr(self, "state", None), "dead_units", None)
        if du:
            return [int(t) for t in du]
        ev = getattr(getattr(getattr(self.state, "observation", None), "raw_data", None), "event", None)
        dead = getattr(ev, "dead_units", None) if ev is not None else None
        return [int(t) for t in (dead or [])]
    ...
    async def begin_episode_via_chatreset(self):
        ...
        # record the current marine tags
    ...
    async def _finish_episode_for_current_agent(self, reason: str, already_reset: bool = False, final_snapshot: Optional[dict] = None):
        # print("_marine_deaths_this_ep:", self._marine_deaths_this_ep)
        # print("_kills_this_ep:", self._kills_this_ep)
    ...
    async def on_step(self, iteration: int):
        ...
        for u in self.enemy_units:  # 当前可见
            self._enemy_tag2type[int(u.tag)] = u.type_id
        # print("self._enemy_tag2type:", self._enemy_tag2type)

        for tag in self._dead_unit_tags():
            if tag in self._dead_seen_tags:
                continue
            self._dead_seen_tags.add(tag)
            
            # marine dead
            if tag in self._alive_marine_tags:
                self._alive_marine_tags.discard(tag)
                self._marine_deaths_this_ep += 1
                continue

            # enemy dead
            t = self._enemy_tag2type.get(tag, None)
            if t in BIO:
                self._kills_this_ep += 1
                self._enemy_tag2type.pop(tag, None)  # 可选：清掉
        
        # print("_marine_deaths_this_ep:",self. _marine_deaths_this_ep)
        # print("_kills_this_ep:",self. _kills_this_ep)
        
        # step reward
        if self._has_acted_this_episode:
        ...
        # print("auto_reset_jump:", auto_reset_jump)
        if auto_reset_jump:
            reason = reason or "auto_reset_jump"
        ...
        if done_timeout:
            ...
            return
        if self. _marine_deaths_this_ep == 3:
            reason = "all_dead"
            await self._finish_episode_for_current_agent(reason)
            return
